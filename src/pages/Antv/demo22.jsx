...
// 将子节点线node隐藏
childrenNode.nodes.forEach(node => node.visible = true);

// 生成子节点的 edges
childrenNode.edges = createEdges(childrenNode.nodes)
// 将子节点线隐藏
childrenNode.edges.forEach(node => node.visible = true);
...
const Tutorial = () => {
  ...
  const initNodeEvent = () => {
    ...
    graph.on('region-image-label:click', e => {
      ...
      // debugger;
      // 展示子节点逻辑
      const childrenLayoutNodes = childrenNode.nodes;
      ...
      let _graphData = {
          nodes: graphData.nodes.concat(childrenLayoutNodes),
          edges: graphData.edges
        }

        graphRef.current.changeData(_graphData);
  ...
  return (
    ...
    >
      ...
      <div ref={leftDom} 
        style={{
          // backgroundColor: 'rgba(111,222,333)', 
          // opacity: 0.99,
          background: 'black',
